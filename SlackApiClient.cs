...
namespace SlackNet
{
    ...
    public class SlackApiClient : ISlackApiClient
    {
        ...
        public SlackApiClient(IHttp http, ISlackUrlBuilder urlBuilder, SlackJsonSettings jsonSettings, string token, string apptoken=null)
        {
            _http = http;
        ...
        public Task<T> Post<T>(string requestUri, object body, CancellationToken? cancellationToken) where T : class =>
            WebApiRequest<T>(() =>
                {
                    var requestMessage = new HttpRequestMessage(HttpMethod.Post, requestUri);
                    if (_apptoken != null && requestUri.Contains("apps.connections.open"))
                    {
                        requestMessage.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _apptoken);
                    }
                    else
                    {
        ...
        private string Url(string apiMethod)
        {
            if (_apptoken != null)
            {
               return _urlBuilder.Url(apiMethod, new Args(), true);
            }
            else
            {
               return _urlBuilder.Url(apiMethod, new Args());
            }
        }
        ...
        private async Task<T> WebApiRequest<T>(Func<HttpRequestMessage> createRequest, CancellationToken? cancellationToken) where T : class
        {
            while (true)
            {
                ...
                {   
                    await Task.Delay(e.RetryAfter ?? TimeSpan.FromSeconds(3)).ConfigureAwait(false);
                }
            }
        }
