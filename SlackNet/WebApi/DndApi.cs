...
public interface IDndApi
{
    ...
    Task<IReadOnlyDictionary<string, DndStatus>> TeamInfo(IEnumerable<string> userIds, CancellationToken cancellationToken = default);
}
...
public class DndApi : IDndApi
{
    ...
    public Task EndDnd(CancellationToken cancellationToken = default) =>
        _client.Post("dnd.endDnd", new Args(), cancellationToken);

    public Task<DndResponse> EndSnooze(CancellationToken cancellationToken = default) =>
        _client.Post<DndResponse>("dnd.endSnooze", new Args(), cancellationToken);

    public Task<OwnDndResponse> Info(CancellationToken cancellationToken = default) =>
        _client.Get<OwnDndResponse>("dnd.info", new Args(), cancellationToken);

    public Task<DndResponse> Info(string userId, CancellationToken cancellationToken = default) =>
        _client.Get<DndResponse>("dnd.info", new Args { { "user", userId } }, cancellationToken);

    public Task<SnoozeResponse> SetSnooze(int numMinutes, CancellationToken cancellationToken = default) =>
        _client.Get<SnoozeResponse>("dnd.setSnooze", new Args { { "num_minutes", numMinutes } }, cancellationToken);

    public async Task<IReadOnlyDictionary<string, DndStatus>> TeamInfo(IEnumerable<string> userIds, CancellationToken cancellationToken = default) =>
