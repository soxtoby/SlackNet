...
namespace SlackNet
{
    public interface ISlackRtmClient : IDisposable
    {
        ...
        Task<ConnectResponse> Connect(bool batchPresenceAware = false, bool manualPresenceSubscription = false, CancellationToken? cancellationToken = null, bool socketmode=false);
        ...
        void SendAck(string envelope_ID);
    ...
    public class SlackRtmClient : ISlackRtmClient
    {
        ...
        /// <param name="batchPresenceAware">Group presence change notices in <see cref="PresenceChange"/> events when possible.</param>
        /// <param name="cancellationToken"></param>
        public async Task<ConnectResponse> Connect(bool batchPresenceAware = false, bool manualPresenceSubscription = false, CancellationToken? cancellationToken = null, bool socketmode=false)
        {
            if (Connected)
            ...
            if (socketmode)
            {
                connectResponse = await _client.Rtm.SocketConnect(manualPresenceSubscription, batchPresenceAware, cancellationToken).ConfigureAwait(false);
            }
            else
            {
        ...
        public void SendAck(string envelopeID)
        {

            System.Console.WriteLine("Trying to send ACK for "+envelopeID);
            Send(new Ack { envelope_id = envelopeID });
        }
