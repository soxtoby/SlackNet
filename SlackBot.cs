...
namespace SlackNet.Bot
{
    public interface ISlackBot : IObserver<BotMessage>
    {
        ...
        /// Connect to Slack.
        /// </summary>
        Task Connect(CancellationToken? cancellationToken = null, bool socketmode=false);
    ...
    public class SlackBot : ISlackBot, IDisposable
    {
        ...
        /// Connect to Slack.
        /// </summary>
        public async Task Connect(CancellationToken? cancellationToken = null, bool socketmode = false)
        {
            // If already connected, client will throw
            var connection = _rtm.Connect(false, false, cancellationToken, socketmode);
            ...
            {
                var aa = new AuthApi(_api);
                var xx = await aa.Test().ConfigureAwait(false);
                Id = xx.UserId;
                Name = xx.User;
                //TODO: SocketMode does NOT return these, we can get them using the bot token and auth.test
        }
    }
        ...
        private async Task<IReadOnlyList<User>> FetchUsers()
        {
            ...
            do
            {
                ...
                cursor = response.ResponseMetadata.NextCursor;
                //Users is rate limited at 20 per minute, so pause 4 seconds simply as 3 seconds would still rate limit at times
